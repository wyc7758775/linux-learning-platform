    ...
    <>
      ...
      <div className={`rounded-2xl border overflow-hidden h-full min-h-0 flex flex-col ${
          ...
          {showCompletionPrompt && (
            <div className={`rounded-xl p-4 border ${
              ...
              <div className={hasNextLevel ? 'space-y-3' : ''}>
                {completionSummary}
                {hasNextLevel && nextLevelButton}
              </div>
            </div>
          )}

          {isCompleted && level.completionKnowledgeCards && level.completionKnowledgeCards.length > 0 && (
            <div className={`rounded-xl p-4 border ${
              isDark
        ...
        {showCompletionPrompt && hasNextLevel && (
          <div className={`lg:hidden sticky bottom-16 flex-shrink-0 mx-4 mb-4 rounded-2xl border px-5 py-4 backdrop-blur-md relative z-50 shadow-[0_-10px_30px_rgba(15,23,42,0.16)] ${
            isDark
