...
const LEVELS: LevelType[] = [
  ...
  // Chapter 6: 脚本编程
  {
    ...
    chapter: 6,
    title: '第一个脚本',
    description: '运维新人接到任务：写一个系统信息报告脚本 report.sh，输出 "System Report" 标题和当前用户名',
    ...
    knowledgeCards: [
      {
        ...
        description: '脚本第一行，告诉系统用哪个解释器执行脚本。#!/bin/bash 使用 bash，#!/bin/sh 使用 sh',
      },
      {
        command: 'bash script.sh',
        description: '直接用 bash 解释器执行脚本，无需可执行权限。等价于 ./script.sh（需要 chmod +x）',
      },
      {
        command: 'whoami',
  ...
  },
  {
    ...
    chapter: 6,
    title: '变量与替换',
    description: '服务器的 IP、端口、部署路径经常变动。用变量定义 SERVER_IP、PORT、DEPLOY_PATH，并输出配置摘要',
    ...
    knowledgeCards: [
      {
        command: '变量赋值',
        description: 'VAR=value（等号两边不能有空格）。引用：$VAR 或 ${VAR}。花括号形式用于拼接：${VAR}_log',
      },
      {
        command: '双引号 vs 单引号',
        description: '"$VAR" 会替换变量值，\'$VAR\' 原样输出。推荐双引号包裹含变量的字符串',
      },
      {
        command: '命令替换 $(...)',
        description: '把命令输出赋给变量：DATE=$(date +%Y-%m-%d)。反引号 `date` 也可以但不如 $() 嵌套方便',
      },
    ],
    validation: { type: 'file_content_contains', expected: '/home/player/config.sh:SERVER_IP' },
    completed: false
  },
  {
    id: 43,
    chapter: 6,
    title: '读取输入',
    description: '部署脚本需要确认目标环境。编写 deploy_env.sh，用 read 询问"请输入目标环境(dev/staging/prod):"，然后输出"Deploying to [环境名]"',
    ...
    knowledgeCards: [
      {
        command: 'read VAR',
        description: '从标准输入读取一行赋给变量 VAR。常用选项：-p "提示" 显示提示，-s 隐藏输入（密码），-t 5 超时 5 秒',
      },
      {
    ...
    validation: { type: 'file_content_contains', expected: '/home/player/deploy_env.sh:read' },
    completed: false
  },
  {
    id: 44,
    chapter: 6,
    title: '条件判断',
    description: '检查 /home/player/app.log 是否存在：存在则输出"Log file found"，不存在则输出"Log file not found"',
    ...
    knowledgeCards: [
      {
        ...
        description: '基本条件判断。[ ] 等价于 test 命令。注意 [ 后和 ] 前必须有空格',
      },
      {
        command: '文件测试操作符',
        description: '-f 文件存在且为普通文件, -d 目录存在, -e 存在, -r 可读, -w 可写, -x 可执行, -s 非空',
        flags: [
      ...
      {
        command: '字符串比较',
        description: '"$a" = "$b" 相等，"$a" != "$b" 不等，-z "$a" 为空，-n "$a" 非空',
      },
      {
        command: '数值比较',
        description: '-eq 等于, -ne 不等于, -gt 大于, -lt 小于, -ge 大于等于, -le 小于等于',
        flags: [
    ...
    validation: { type: 'file_content_contains', expected: '/home/player/check.sh:if' },
    completed: false
  },
  {
    id: 45,
    chapter: 6,
    title: '退出码与逻辑',
    description: '编写脚本 safe_rm.sh：先用 test -f 检查文件是否存在，存在则删除并输出"Deleted"，不存在则输出"File not found"并以 exit 1 退出',
    ...
    knowledgeCards: [
      {
        command: '$?（退出码）',
        description: '上一条命令的返回值。0 表示成功，非 0 表示失败。脚本中用 exit N 返回指定退出码',
      },
      {
        command: 'exit N',
        description: '终止脚本并返回退出码 N。exit 0 表示成功，exit 1 表示一般错误',
      },
      {
        command: '&& 和 ||（短路逻辑）',
        description: 'cmd1 && cmd2：cmd1 成功才执行 cmd2。cmd1 || cmd2：cmd1 失败才执行 cmd2',
      },
    ],
    validation: { type: 'file_content_contains', expected: '/home/player/safe_rm.sh:exit' },
    completed: false
  },
  {
    id: 46,
    chapter: 6,
    title: '循环遍历',
    description: '编写 disk_check.sh，用 for 循环检查 /home、/var、/tmp 三个目录的磁盘使用情况，对每个目录输出 "Checking [目录] ... done"',
    ...
    knowledgeCards: [
      {
        ...
        description: '遍历列表中的每个元素。列表可以是空格分隔的值、命令替换结果或通配符展开',
      },
      {
        command: 'for ((i=0; i<N; i++)); do ... done',
        description: 'C 风格 for 循环，适合需要数值递增的场景。双括号内可以使用 < > <= >= 等数学比较',
      },
      {
        command: 'seq 1 10',
    ...
    validation: { type: 'output_contains', expected: 'Checking /home ... done' },
    completed: false
  },
  {
    id: 47,
    chapter: 6,
    title: '循环读取',
    description: '编写 log_stats.sh，逐行读取 /var/log/nginx/access.log，统计请求总数并输出"Total requests: [数量]"',
    ...
    knowledgeCards: [
      {
        ...
        description: '逐行读取文件，每次循环 LINE 变量包含一行内容。文件末尾自动退出循环',
      },
      {
        command: 'cat file | while read LINE',
        description: '管道方式逐行读取。注意：管道中的 while 在子 shell 中运行，循环内的变量修改不会影响外部',
      },
      {
        command: '算术运算 $(())',
        description: '$((a+b)) 整数运算。支持 + - * / %（取模）。赋值：count=$((count+1)) 或 ((count++))',
      },
    ],
  ...
  },
  {
    ...
    chapter: 6,
    title: '函数封装',
    description: '编写 monitor.sh，定义 check_service() 函数：接收服务名参数 $1，输出"Checking [服务名]... OK"。调用该函数检查 nginx 和 sshd 两个服务',
    ...
    knowledgeCards: [
      {
        command: '函数定义与调用',
        description: 'func_name() { commands; } 或 function func_name { commands; }。调用时不加括号：func_name arg1 arg2',
      },
      {
        command: '函数参数',
        description: '函数内用 $1 $2 ... 访问参数，$# 参数个数，$@ 所有参数。与脚本参数用法相同',
      },
      {
        command: 'return vs echo',
        description: 'return N 返回退出码（0-255），用 $? 获取。echo "value" 返回字符串，用 result=$(func) 捕获',
      },
    ],
  ...
  },
  {
    ...
    chapter: 6,
    title: '字符串处理',
    description: '/home/player/app.conf 是一个配置文件。编写 parse.sh，提取其中 SERVER_NAME 的值并输出"Server: [值]"',
    ...
    knowledgeCards: [
      {
        command: '参数扩展（字符串截取）',
        description: '${var#pattern} 删最短前缀, ${var##pattern} 删最长前缀, ${var%pattern} 删最短后缀, ${var%%pattern} 删最长后缀',
        flags: [
      ...
      },
      {
        ...
        description: '按分隔符切割每行并提取字段。cut -d= -f2 提取等号后的第二个字段。cut -d: -f1,3 提取第 1、3 字段',
      },
      {
        command: 'awk -F分隔符 \'{print $N}\'',
        description: '比 cut 更强大的文本处理工具。awk -F= \'{print $2}\' 提取等号后内容，支持条件、循环、函数',
      },
    ],
  ...
  },
  {
    ...
    chapter: 6,
    title: '服务器健康检查',
    description: '综合实战！编写 healthcheck.sh，用函数+循环检查 CPU 负载、磁盘空间、内存使用、关键进程，输出"Health Check Report"和各项结果',
    ...
    knowledgeCards: [
      {
        command: '脚本最佳实践',
        description: '1. set -e 遇错即停 2. set -u 使用未定义变量报错 3. 用函数组织逻辑 4. 关键操作加日志输出',
      },
      {
        command: 'df -h',
        description: '显示磁盘使用情况。-h 人类可读格式（KB/MB/GB）。df -h / 查看 / 分区用量',
      },
      {
        command: 'free -m',
        description: '显示内存使用情况。-m 以 MB 为单位。free -m | awk \'/Mem/{print $3/$2*100}\' 获取内存使用百分比',
      },
      {
        command: 'ps aux | grep',
        description: '检查特定进程是否运行。pgrep -x nginx 比 ps | grep 更精确（不会匹配 grep 自身）',
      },
    ],
